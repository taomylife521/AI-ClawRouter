 ...
 */
async function waitForProxyHealth(port, timeoutMs = 3000) {
...
function injectModelsConfig(logger) {
    ...
    if (existsSync(configPath)) {
        try {
            ...
            logger.info(`Skipping config injection (corrupt file): ${err instanceof Error ? err.message : String(err)}`);
    ...
    if (!providers.blockrun) {
        ...
        const currentModelIds = new Set(Array.isArray(currentModels) ? currentModels.map((m) => m?.id).filter(Boolean) : []);
        const expectedModelIds = OPENCLAW_MODELS.map((m) => m.id);
        const needsModelUpdate = !currentModels ||
...
async function startProxyInBackground(api) {
    ...
    const maxCostPerRunUsd = typeof api.pluginConfig?.maxCostPerRun === "number"
        ? api.pluginConfig.maxCostPerRun
        : undefined;
    const maxCostPerRunMode = api.pluginConfig?.maxCostPerRunMode === "strict" ? "strict" : "graceful";
    if (maxCostPerRunUsd !== undefined) {
        api.logger.info(`Cost cap: $${maxCostPerRunUsd.toFixed(2)} per session (mode: ${maxCostPerRunMode})`);
    }
        ...
        if (currentChain === "solana" && (balance.isEmpty || balance.isLow)) {
            try {
                ...
                if (solLamports > 10000000n) {
                    ...
                    api.logger.info(`You have ${sol.toFixed(2)} SOL — swap to USDC: https://jup.ag/swap/SOL-USDC`);
                }
            }
            catch {
    ...
    return {
        ...
        handler: async (ctx) => {
            ...
            if (subcommand === "export") {
                ...
                try {
                    if (existsSync(MNEMONIC_FILE)) {
                        ...
                        if (mnemonic) {
                            ...
                            lines.push("", "**Solana:**", `  Address: \`${signer.address}\``, `  (Derived from mnemonic below)`, "", "**Mnemonic (24 words):**", `\`${mnemonic}\``, "", "CRITICAL: Back up this mnemonic. It is the ONLY way to recover your Solana wallet.");
                        }
                    }
                }
                catch {
                ...
                lines.push("", "**To restore on a new machine:**", "1. Set the environment variable before running OpenClaw:", `   \`export BLOCKRUN_WALLET_KEY=${walletKey}\``, "2. Or save to file:", `   \`mkdir -p ~/.openclaw/blockrun && echo "${walletKey}" > ~/.openclaw/blockrun/wallet.key && chmod 600 ~/.openclaw/blockrun/wallet.key\``);
                if (hasMnemonic) {
                    lines.push("3. Restore the mnemonic for Solana:", `   \`echo "<your mnemonic>" > ~/.openclaw/blockrun/mnemonic && chmod 600 ~/.openclaw/blockrun/mnemonic\``);
                }
            ...
            try {
                ...
                if (stats.totalRequests > 0) {
                    const modelLines = Object.entries(stats.byModel)
                        ...
                        .slice(0, 8)
                        .map(([model, data]) => `  ${model.length > 30 ? model.slice(0, 27) + "..." : model}  ${data.count} reqs  $${data.cost.toFixed(4)}`);
                    usageSection = [
                        "",
                        `**Usage (${stats.period}):**`,
...
const plugin = {
    ...
    register(api) {
        ...
        try {
            ...
            api.logger.warn(`Failed to register partner tools: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        createWalletCommand()
            ...
            .catch((err) => {
            api.logger.warn(`Failed to register /wallet command: ${err instanceof Error ? err.message : String(err)}`);
        });
        ...
        createExcludeCommand()
            ...
            .catch((err) => {
            api.logger.warn(`Failed to register /exclude command: ${err instanceof Error ? err.message : String(err)}`);
        });
            ...
            stop: async () => {
                ...
                if (activeProxyHandle) {
                    try {
                        ...
                        api.logger.warn(`Failed to close proxy: ${err instanceof Error ? err.message : String(err)}`);
                    }
        ...
        if (!isGatewayMode()) {
            ...
            resolveOrGenerateWalletKey()
                ...
                .catch((err) => {
                api.logger.warn(`Failed to initialize wallet: ${err instanceof Error ? err.message : String(err)}`);
            });
        ...
        startProxyInBackground(api)
            ...
            .catch((err) => {
            api.logger.error(`Failed to start BlockRun proxy: ${err instanceof Error ? err.message : String(err)}`);
        });
    },
};
